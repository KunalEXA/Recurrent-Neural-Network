...
X = []
Y = []
with open('foo.csv','rt', encoding='utf8') as csvfile:
	spr = csv.reader(csvfile, delimiter=' ', quotechar='|')
	for row in spr:
		r = list(map(float,row[0].split(',')))
		X.append((r[0],r[1]))
		Y.append((r[2],r[3]))
...
for _ in range(100):
    ...
    error = 0
    #To determine the learning rate we see what is the improvement in error. For this we need to perform a simple feed forward.
    for x,y in zip(X, Y):
        t = feedforward(x)
        error +=(0.5*(t[0]-y[0])*(t[0]-y[0]))+(0.5*(t[1]-y[1])*(t[1]-y[1]))
    errors.append(error)
    
#errors are plotted here
xlabel('Epochs')
ylabel('Mean Squared Error')
title('Learning rate with step size 0.5')
plot(errors)